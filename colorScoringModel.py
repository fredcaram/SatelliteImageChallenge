...
def getImageDfWithScore(filePath, score, redimImageSize):
...
def trainETScoreClassificationModel(scoreDf, printModelScore:bool):
    X = np.array(scoreDf.loc[:, ["NewR", "NewG", "NewB"]].values)
    y = np.array(scoreDf.loc[:, ["score"]].values)
    y = np.ravel(y)
...
def GetImageScore(imgHelper, pixelsToRemove, scoreModel):
    rgbArray = imgHelper.prepImageArray()
    rgbArray = rgbArray[np.logical_not(pixelsToRemove)]
    prediction = scoreModel.predict(rgbArray)
